...
var Db = module.exports = function(dbname, server) {
  ...
  var iface = {
    ...
    collection: function(name, options, callback) {
      ...
      var collection = getCollection(name, options.pkFactory);
  ...
  function getCollection(name, pk) {
    var instance = _.find(backingstore.collections, {name:name} );
    if(instance) {
      instance.__setPkFactory(pk)
      return instance;
    }

    var state = new CollectionState(name, undefined, pk);
